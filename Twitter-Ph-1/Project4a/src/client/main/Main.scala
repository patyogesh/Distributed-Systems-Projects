...
object Main {
  ...
  def main(args: Array[String]) {
    ...
    for (i <- 1 to clients) {
      var client = system.actorOf(Props(new ClientActor(serverAddress, followers((i % sampleSize) - 1), numberOfTweetsPerDay((i % sampleSize) - 1), i * offset)), "Client" + i)
    }
  }
}